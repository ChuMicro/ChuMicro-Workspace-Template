...
| `python run.py deploy --all-things` | Walk `workspace.yml`'s `deploy_targets:` mapping and deploy each thing to its declared device(s).  Mutually exclusive with positional names / `--device` / `--runtime` / `--all-devices`. |
